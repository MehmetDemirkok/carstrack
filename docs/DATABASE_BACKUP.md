...
- Yedek başarıyla alındığında **sıkıştırılmış dosya ek olarak** `BACKUP_NOTIFY_EMAIL` adresine (tanımlı değilse `mehmetdemirkok@gmail.com`'a) e-posta ile de gönderilir — Supabase projesi tamamen kaybolsa bile elinizde bir kopya kalır. Yedekleme başarısız olursa ayrı bir uyarı e-postası gider.
...
1. **Dosyayı bul:** E-postanıza gelen `.json.gz` ekini indirin, ya da Supabase Dashboard → Storage → `db-backups` → `backups/` klasöründen ilgili tarihi indirin.
